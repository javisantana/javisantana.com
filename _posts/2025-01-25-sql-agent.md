---
name: Building a simple SQL Agent from Scratch
...
I wanted to create a really simple SQL Agent to teach myself how to do it, no libraries to simplify the process, just a bash script using the [llm cli tool](https://llm.datasette.io/en/stable/index.html). I don't actually like bash so much but it's somehow limited so that allows me to focus on the actual problem. 
...
There are just two changes to the agent:
1) we pass to the prompt a list of columns for each table with the type (using `information_schema.columns` table)
2) we pass the database to the agent so it can use the data inside the database.
...
I see many ways to fix this:
 ...
 1. Give more info about the data (like a sample) so in the case of the example, it can actually see how the timestamp look like
 1. Put human in the loop so it can give more context, save the context for future runs and so on. Actually that's what people are doing, check this example from Uber on how they do it: [QueryGPT](https://www.uber.com/en-TW/blog/query-gpt/).
